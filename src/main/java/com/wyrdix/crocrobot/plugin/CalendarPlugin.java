...
import com.wyrdix.crocrobot.command.calendar.*;
...
import com.wyrdix.crocrobot.plugin.calendar.ComplexCalendarElement;
import com.wyrdix.crocrobot.plugin.calendar.impl.CalendarInstanceImpl;
...
public class CalendarPlugin extends ListenerAdapter implements Plugin {
    ...
    private static ItemComponent[] generateButton(int week) {
        if (week == 1) {
            return new ItemComponent[]{Button.primary("crocrobot_week_" + (week + 1), "Semaine Suivante")};
        } else {
            return new ItemComponent[]{Button.primary("crocrobot_week_" + (week - 1), "Semaine Précédente"), Button.primary("crocrobot_week_" + (week + 1), "Semaine Suivante")};
        }
    }
    ...
    public void onButtonInteraction(ButtonInteractionEvent event) {
        ...
        if (button.getId() == null || !button.getId().startsWith("crocrobot_week_")) return;
        ...
        int week = Integer.parseInt(button.getId().substring("crocrobot_week_".length()));
    ...
    static final class ImageGenerator {
        ...
        public static BufferedImage getCalendar(int week, KUser user) {
            ...
            CalendarPlugin plugin = CroCroBot.getPlugin(CalendarPlugin.class);
    ...
    public void onEnable() {
        ...
        CroCroBot.getJDA().addEventListener(this);
    }
